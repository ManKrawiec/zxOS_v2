...
namespace Keyboard {
    ...
    bool DecodeScancode(u8 scancode, Event* event) {
        char c = 0;
        ...
        if (normal_lowercase[scancode]) {
            ...
            c = is_shift_pressed ? (char)normal_uppercase[scancode] : (char)normal_lowercase[scancode];
        } else if (keypad[scancode]) {
            event->type = EventType::Keypad;
            c = (char)keypad[scancode];
        } else if (special[scancode]) {
            event->type = EventType::Special;
            c = (char)special[scancode];
            if (c == SpecialKeyCode::LeftShift || c == SpecialKeyCode::RightShift) {
    ...
    void EventHandle() {
        Event event;
        (void)DecodeScancode(IO::Input::Byte(0x60), &event);
    }
}
